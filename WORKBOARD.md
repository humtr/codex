...
- Fresh remote authority baseline for the current bundle is exact `rewrite/rust-core=201462ac2b718e926417fe04b480aea133ed795f`.
- Fresh public readback on 2026-09-23 supersedes the older sequence-17 / NO-EMOJI-in-progress routing notes retained below as history: `main=30813a401bcbf2001b67a4e1af6e22b92da9ca49`, public stable generation `local-hosted-0-155-1-9300a68852c8-no-emoji-output-r3`, and hosted production run `35518125062` completed successfully. This bundle does not authorize another public promotion or a live `codex update`.
- Active implementation bundle: **SHARED-CONVERSATION-STATE (SCS)**. User decision on 2026-09-23 changes the Manager profile model from profile-owned conversations to profile-specific auth/config/runtime identity over one user-global local conversation state. `$HOME/.codex` is the canonical shared-state root; custom profile `CODEX_HOME` remains isolated. The implementation must preserve official-prebuilt upstream runtime qualification and must not introduce a custom upstream source build.
- **Thin-wrapper invariant:** steady-state Manager responsibility stops at execution-identity selection, bounded profile-local environment/configuration, compatibility topology, and `exec` of the official-prebuilt upstream runtime. Conversation/thread/session storage semantics remain upstream-owned. After migration is complete, ordinary Manager launch/list/resume paths must not become a second thread-store implementation, routinely parse SQLite or transcript contents, infer conversation ownership, or maintain a parallel session index. A future change that requires those behaviors is an architectural-regression signal unless a narrowly bounded upstream-compatibility exception is documented and proved necessary.
- SCS vertical proof order is: **SCS-1 contract/topology -> SCS-2 launch and SQLite routing -> SCS-3 upstream-owned session semantics and Manager session-surface retirement -> SCS-4 bounded recent-conversation migration/rollback machinery -> SCS-5 disposable multi-profile runtime acceptance -> SCS-6 live bounded user-state migration acceptance**. Product-code mutation must not advance past a red slice. SCS-6 may begin only after the preceding source/disposable gates are green, all live Codex/app-server writers are quiesced, and a restorable backup is verified. Public release/live-runtime mutation remains separately gated.
- Current read-only device inventory is migration evidence, not authority to rewrite data: default plus legacy `.codex-profiles/*` contain heterogeneous state schemas. The accepted migration scope is intentionally small: **from each legacy user profile, import at most the five most-recent distinct conversations by last activity** into the current canonical shared state. Selection must be deterministic and fail closed for a profile if safe activity ordering cannot be established. For each selected conversation, migrate only the dependency closure required for faithful upstream resume/history behavior under the current schema; identical cross-profile copies may deduplicate, while divergent state for the same conversation/thread identity must fail closed rather than overwrite. Whole-database replacement, blind `INSERT OR REPLACE`, and a permanent generic legacy-merger subsystem are prohibited. Non-selected legacy conversations remain untouched in the verified backup/source profile and are outside canonical import scope.
- **SCS-4/SCS-5 current proof (2026-09-26):** the bounded migration is now an operator-only one-shot tool, not a steady-state Manager subsystem. `scripts/shared_state_migrate.py` reads legacy `state_5.sqlite` only to select at most five recent **recoverable** conversations per profile and to detect unsupported hidden thread-scoped dependencies; it never writes or merges legacy/canonical SQLite. The only migration payload is authoritative rollout JSONL. Identical cross-profile rollout copies deduplicate by thread identity plus content hash/history mode; divergent content fails closed. Broken legacy aliases count as stale, safe cross-profile legacy aliases may resolve only inside `.codex-profiles/*/{sessions,archived_sessions}`, and a selected canonical legacy symlink is normalized to a regular canonical rollout with journaled rollback. Dedicated `logs_2.sqlite` and rebuildable `thread_history_1.sqlite` projections are not copied. `finalize` delegates metadata read-repair, paginated history materialization, and explicit-name restoration to official upstream app-server `thread/list`, `thread/resume`, `thread/turns/list`, and `thread/name/set`; once upstream activation begins, payload-only rollback is forbidden and SCS-6 must restore the verified full canonical backup. Migration unit coverage is 9/9 PASS. Fresh live read-only planning selected 23 recoverable profile rows -> 17 unique threads (16 copies + 1 alias normalization, 9 named, 7 stale skipped) with stable selection. Disposable official-runtime acceptance proved 16/16 regular-copy threads list/resume/turns successfully and 12/12 eligible paginated histories fully rematerialize. One selected 173,101,495-byte paginated rollout exceeds the tdev exec hard `RLIMIT_FSIZE=262144` blocks and therefore cannot be regular-copied inside the diagnostic harness; its fresh source hash/session identity match the plan and its existing source projection is present but behind the authoritative rollout, which reinforces rather than weakens rollout-authoritative migration. This harness limit is not authority for live mutation; SCS-6 remains gated on quiescence and a verified restorable full backup.
