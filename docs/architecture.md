...
대표 설치 흐름은 `install.sh` -> `bin/install-runtime.sh setup` -> support file 설치 -> launcher 설치 -> raw package 확보 또는 cached raw로 repair -> candidate runtime build -> smoke test -> atomic promotion -> state/registry 기록 순서다. 의존성 설치가 먼저 끝나야 `npm pack`, `python3`, `tar`, `curl`, `bash`를 신뢰할 수 있고, support file 설치가 먼저 끝나야 runtime rebuild가 최신 `bwrap`/`rg` 호환 도구를 runtime tree에 넣을 수 있다.
...
state는 active tuple과 last-known-good verified tuple을 함께 기록한다. runtime prune는 active tuple과 verified tuple을 우선 보호하고, verified tuple은 smoke test를 통과한 runtime만 가리켜야 한다.
