...
- Named profile의 `plugins` 항목이 없으면 default `~/.codex/plugins`를 공유 symlink로 연결하는 변경이 working tree에 구현되어 있다.
- Termux profile 실행 때 workspace-write network access를 true로 보장하는 변경이 working tree에 구현되어 있다.
...
- 자동화된 test suite가 없다. 현재 검증은 shell syntax, targeted smoke test, `doctor`, 수동 DNS 분리 확인에 의존한다.
- `install.sh`, `bin/install-runtime.sh`, `tools/build-runtime.py`, `tools/bwrap-termux-compat.py`의 regression test가 파일로 고정되어 있지 않다.
...
Working tree에는 profile plugin 공유 변경과 이 프로젝트 안내 문서가 포함되어 있다. 문서 생성 자체는 제품 기능 완료로 세지 않으며, 코드 기능으로는 profile plugin 공유가 현재 미커밋 변경이다.
