...
이 명령은 public launcher와 support scripts를 갱신하지만 upstream Codex package를 새로 받지 않는다. `lib/`, `tools/`, launcher 관련 변경을 live 설치에 반영할 때 사용한다.
...
`doctor --json`에서 runtime, raw, bwrap, rg, resolver, cert, state, registry, DNS patch 관련 check가 모두 true여야 live 설치가 정상이다. 네트워크 증상은 `doctor`만으로 결론 내리지 말고 sandbox 밖 DNS query와 profile network 설정을 같이 확인한다.
...
관리형 marker가 있는 `$PREFIX/bin/codex`와 `$PREFIX/bin/bwrap`만 제거한다. marker 없는 launcher는 setup/update 때 만들어 둔 backup이 있으면 복구한다. runtime tree는 제거하지만 state directory는 backup 추적을 위해 남긴다.
...
- `CODEX_NATIVE_BWRAP_MODE`: `compat` 기본값이며 `real` 또는 `upstream`이면 real bwrap 실행을 요구한다.
