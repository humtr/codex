...
Immutable store tuple은 publish 후 rewrite하지 않는다. 같은 tuple id를 다시 쓰려면 전체 tree 내용과 실행 권한이 동일해야 하며, 다르면 collision로 취급해 activation을 거부한다.

raw/runtime promotion은 candidate staging 뒤 immutable artifact publish와 rollback-safe pointer/metadata activation으로 처리해야 한다. 중간 실패가 active current/verified/raw pointer나 state/registry를 엇갈리게 만들면 안 된다.

runtime prune는 state/registry뿐 아니라 실제 current/verified/raw pointer target도 보호해야 한다.

Legacy cache migration은 best-effort다. builder, patch policy, support shim, raw/runtime hash, DNS-only patch가 모두 맞는 entry만 새 store로 가져오며, invalid runtime이나 missing raw cache는 skip하되 active runtime을 막지 않는다.
