...
from . import _legacy_source as legacy
...
def prepare_support_install(
    *,
    ...
    legacy.require_wrapper_source(source_root, "Wrapper source")
    ...
    commit = wrapper_commit or legacy.source_commit(source_root)
...
def _populate_support(
    *,
    ...
    _write_notify_compat(target / "codex-turn-notify.sh", manager_link, hook=True)
    _write_notify_compat(target / "termux-notify.sh", manager_link, hook=False)
...
def _validate_support(target: Path) -> None:
    ...
    shell_paths.extend((target / "codex-turn-notify.sh", target / "termux-notify.sh"))
    ...
    result = subprocess.run(
        [shutil.which("python3") or "python3", "-B", str(target / "libexec/notify"), "self-test"],
        check=False,
        capture_output=True,
        text=True,
        timeout=10,
    ...
    if result.returncode != 0:
        raise IntegrityError("notification self-test failed: " + (result.stderr or result.stdout).strip())
