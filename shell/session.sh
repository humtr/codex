...
codex_session() {
    ...
    # Session profile selection must not inherit another profile's auth home.
    if codex_termux_cmd profile-is-default --profile "${CODEX_SESSION_TARGET_PROFILE:-default}"; then
        unset CODEX_HOME
    fi
