...
if [ -z "${CODEX_TERMUX_SOURCE_DIR:-}" ]; then
    if [ -e "$CODEX_TERMUX_MANAGER_DIR/source" ]; then
        CODEX_TERMUX_SOURCE_DIR="$CODEX_TERMUX_MANAGER_DIR/source"
    else
        CODEX_TERMUX_SOURCE_DIR="$CODEX_TERMUX_ROOT/source-snapshot"
    fi
fi
...
if [ -z "${CODEX_TERMUX_RUNTIME_BUILDER:-}" ]; then
    if [ -f "$CODEX_TERMUX_MANAGER_DIR/libexec/build-runtime.py" ]; then
        CODEX_TERMUX_RUNTIME_BUILDER="$CODEX_TERMUX_MANAGER_DIR/libexec/build-runtime.py"
    else
        CODEX_TERMUX_RUNTIME_BUILDER="$CODEX_TERMUX_MANAGER_DIR/build-runtime.py"
    fi
fi
...
CODEX_TERMUX_SHELL_LIB="${CODEX_TERMUX_SHELL_LIB:-$CODEX_TERMUX_SHELL_DIR/loader.sh}"
CODEX_TERMUX_WRAPPER_ROOT="${CODEX_TERMUX_WRAPPER_ROOT:-$(cd "$CODEX_TERMUX_SHELL_DIR/.." && pwd)}"

codex_source_domain() {
    local path="$CODEX_TERMUX_SHELL_DIR/$1.sh"
    [ -r "$path" ] || {
