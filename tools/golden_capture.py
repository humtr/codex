...
The harness intentionally uses only the Python standard library so it can run in
portable CI and in Termux before the Rust migration begins.
"""
...
def validate_contract(path: Path) -> int:
    ...
    if contract.get("schema") != SCHEMA_VERSION:
        raise GoldenError(f"migration contract schema must be {SCHEMA_VERSION}")
    for field in ("feature_boundaries", "policy_boundaries", "migration_phases"):
        value = contract.get(field)
        if not isinstance(value, list) or not value:
            raise GoldenError(f"migration contract field {field} must be a non-empty list")
    language_boundaries = contract.get("language_boundaries")
    if not isinstance(language_boundaries, dict):
        raise GoldenError("migration contract language_boundaries must be an object")
    for language in ("shell", "python", "c", "rust", "external_executables"):
        if language not in language_boundaries:
            raise GoldenError(f"migration contract is missing language boundary: {language}")
    golden = contract.get("golden_behavior")
    if not isinstance(golden, dict):
        raise GoldenError("migration contract golden_behavior must be an object")
    ...
    if not isinstance(observables, list) or not all(isinstance(item, str) for item in observables):
        raise GoldenError("migration contract golden observables must be a list of strings")
    missing = sorted(REQUIRED_CONTRACT_OBSERVABLES - set(observables))
    if missing:
        raise GoldenError("migration contract is missing observables: " + ", ".join(missing))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    group.add_argument("--validate-contract", type=Path, help="migration contract JSON to validate")
