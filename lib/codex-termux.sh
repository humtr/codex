...
for CODEX_TERMUX_DOMAIN in state prompt exec store runtime repair version notify doctor profile use remove session dispatch; do codex_source_domain "$CODEX_TERMUX_DOMAIN" || return $?; done
